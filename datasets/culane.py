...
from datasets.utils.affinity_fields import generateAFs
...
def get_lanes_culane(seg_out, samp_factor):
    ...
    for idx, t_id in enumerate(lane_ids):
        ...
        for y_op in range(seg_out.shape[0]):
            ...
            if x_op.size > 0:
                x_op = np.mean(x_op)
                x_ip = x_op * samp_factor
                y_ip = y_op * samp_factor
...
class CULane(Dataset):
    ...
    def create_index(self):
        ...
        list_file = os.path.join(self.data_dir_path, "list", "{}.txt".format(self.image_set))
...
if __name__ == '__main__':
    img_dir = 'D:\dataset\culane'
    data = CULane(img_dir, image_set='train', random_transforms= True)
    data_length = data.__len__()
    for i in tqdm(range(data_length)):
        img, seg, msk, af = data.__getitem__(i)
